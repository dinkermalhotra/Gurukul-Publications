...
protocol PrimarySelectedBook{
    func onSelectedBook(pBookList:[PrimarySchoolData])
}
protocol IndividaulSelectedBook{
    func onIndSelectedBook(iBookList:[PrimarySchoolData])
}
protocol GroupSelectedBook{
    func onGrpSelectedBook(gBookList:[SelectedBooks])
}
var primarySelectedBook : PrimarySelectedBook?
var individaulSelectedBook : IndividaulSelectedBook?
var groupSelectedBook : GroupSelectedBook?
...
class SampleListViewController: UIViewController{
    ...
    @IBAction func submitBtnClicked(_ sender: UIBarButtonItem) {
        ...
        if(pBookList.count > 0 || iBookList.count > 0 || gBookList.count > 0){
            if let nextVC = UIStoryboard(name: "Main", bundle: nil).instantiateViewController(withIdentifier: "SelectedBookViewController") as? SelectedBookViewController{
...
extension SampleListViewController:PrimarySelectedBook,IndividaulSelectedBook,GroupSelectedBook {
    func onGrpSelectedBook(gBookList: [SelectedBooks]) {
